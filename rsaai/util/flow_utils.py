...
"""
centre crop to 320:224 and resize to 320 x 224
"""
def upsample2d_flow_320_224(flow, mode="bilinear", if_rate=False):
    h, w = flow.size(2), flow.size(3)
    target_ratio = 320 / 224
    if w / h > target_ratio:
        w = int(h * target_ratio)
        cropped_flow = center_crop(flow, (h, w))
    elif w / h < target_ratio:
        h = int(w / target_ratio)
        cropped_flow = center_crop(flow, (h, w))
        
    resized_flow = F.interpolate(cropped_flow, [224, 320], mode=mode, align_corners=True)
    if if_rate:
        _, _, h_, w_ = flow.size()
        u_scale = (320 / w_)
        v_scale = (224 / h_)
        ...
        u = u * u_scale
        v = v * v_scale
        resized_flow = torch.cat([u, v], dim=1)
    return resized_flow
